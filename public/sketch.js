...
let BRUSHFULLSPEED = Math.floor(getRandomFromInterval(BRUSHFULLSPEEDMIN, BRUSHFULLSPEEDMAX) * 10) / 10;
console.log("BRUSHFULLSPEED: " + BRUSHFULLSPEED);

let BRUSHFIBRESIZE = Math.floor(getRandomFromInterval(0.2, 0.4) * 100) / 100; // 0.4;
...
let DISTANCE_BETWEEN_LINES_MULTIPLIER = Math.floor(getRandomFromInterval(1, 10));  // 6, 16
console.log("DISTANCE_BETWEEN_LINES_MULTIPLIER: " + DISTANCE_BETWEEN_LINES_MULTIPLIER);
...
function setup() {
  ...
  BRUSHSIZEMIN = Math.floor(BRUSHSIZEMIN / RESCALINGCONSTANT * DOMINANTSIDE * 10) / 10;
  BRUSHSIZEMAX = Math.floor(BRUSHSIZEMAX / RESCALINGCONSTANT * DOMINANTSIDE * 10) / 10;
  BRUSHFIBRESIZE = Math.floor(BRUSHFIBRESIZE / RESCALINGCONSTANT * DOMINANTSIDE * 10) / 10;
  HATCHOFFSET = Math.floor(HATCHOFFSET / RESCALINGCONSTANT * DOMINANTSIDE * 10) / 10;
  DISTANCE_BETWEEN_LINES_MULTIPLIER = Math.round(DISTANCE_BETWEEN_LINES_MULTIPLIER / RESCALINGCONSTANT * DOMINANTSIDE);
  DISTANCE_BETWEEN_LINES = Math.floor(BRUSHSIZEMAX * DISTANCE_BETWEEN_LINES_MULTIPLIER);
